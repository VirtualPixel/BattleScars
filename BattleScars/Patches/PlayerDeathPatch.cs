...
namespace BattleScars.Patches
{
    ...
    // declared. The slow tick would catch up on its next pass, but the dead
    // head is teleported in by PlayerDeathDone before then, and it reads
    // cosmetics off its own PlayerCosmetics rather than the body's. Without
    // this prefire the dead head wears the saved loadout for a beat.
